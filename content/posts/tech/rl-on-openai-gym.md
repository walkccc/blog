...
為了避免跨平台上安裝的問題，我使用 Google Colab 的雲端平台來 [demo](https://colab.research.google.com/drive/1nellAKykrJOwySQZqZ8eagTmkR0PSXEE)。
...
在 [Human-level control through deep reinforcement learning](https://web.stanford.edu/class/psych209/Readings/MnihEtAlHassibis15NatureControlDeepRL.pdf) 中，提供了三種訓練 DQN 的 tips：
...
1. Freeze target Q-network：即建立兩種 Q-network，
   ...
   - 訓練目標 target network 若只訓練一個 nn，每更新一次時，不但正在訓練的 $Q(s, a)$ 在變，目標 $Q(s', a')$ 也在變，這樣是無法收斂的！
1. Clip rewards：限縮 reward 的值，以利 backpropagation 中能穩定地計算 gradient。
