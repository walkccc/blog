...
The fix is one line: **a product needs an address, not an account.** Google Workspace charges per _user_, not per domain, so a single seat can legitimately own mail for every domain you'll ever buy — add the domain as a secondary domain, add `support@` as an alias on your one user, and turn on _Send mail as_ so you can reply wearing the product's name.
...
Two limits worth knowing up front: 30 aliases per user, so pick one public address per product rather than reflexively creating `support@`, `hello@`, and `press@` on every domain; and a domain can belong to only one Workspace at a time, so decide before you point MX anywhere. No Workspace? Cloudflare Email Routing is the $0 version of all of this — same central inbox, with the tradeoff that you'll need an SMTP relay to reply as the product.
...
解法只有一句：**產品需要的是一個地址，不是一個帳號**。Google Workspace 是按**使用者**收費，不是按網域，所以一個席次就能名正言順地擁有你未來會買的每一個網域的郵件——把網域加成附加網域（secondary domain），在你那唯一的使用者底下加一個 `support@` 別名，再打開**以其他地址發送郵件**，你就能用產品的名字回信。
...
- **DKIM 是每個網域各一把（第 4 步）**。加了附加網域**不等於**它通過驗證——Google 是每個網域各生一把金鑰，而跳過這步就決定了你的回信會不會進垃圾桶。SPF（`v=spf1 include:_spf.google.com ~all`）和 `_dmarc` 也要每個網域各設一份，從 `p=none` 加一個報告收件地址開始，看一週報告之後再收緊。
...
有兩個上限值得先知道：每個使用者 30 個別名，所以每個產品挑一個對外地址就好，不要反射性地在每個網域都開 `support@`、`hello@`、`press@`；以及一個網域同時只能屬於一個 Workspace，所以在把 MX 指出去之前先決定好。沒有 Workspace 的話，這一切的免費版是 Cloudflare Email Routing——一樣的集中收件匣，代價是你需要接一個 SMTP relay 才能用產品地址回信。
