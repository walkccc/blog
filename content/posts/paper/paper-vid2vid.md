...
1. 過去 \\(t - L\\) 到 \\(t - 1\\) 共 \\(L\\) 個生成的圖片 \\(\\tilde{\\textbf x}\_{t - L}^{t - 1}\\)
...
- \\(\\tilde{\\textbf h}\_t = H(\\tilde{\\textbf x}\_{t - L}^{t - 1}, \\textbf s\_{t - L}^t)\\)：模糊後的圖片（generated from scratch）。
...
令 $\textbf w\_{t - K}^{t - 2}$：$K$ 個連續圖片 $\textbf x\_{t - K}^{t - 1}$ 的 $K - 1$ 個光流

$D\_V$ output

- $1$ for a true pair $(\textbf x\_{t - K}^{t - 1}, \textbf w\_{t - K}^{t - 2})$
- $0$ for a fake pair $(\tilde{\textbf x}\_{t - K}^{t - 1}, \textbf w\_{t - K}^{t - 2})$

另外，我們介紹兩個 sampling operators

- $\phi\_I$（random image sampling operator）：$\phi_I(\textbf x\_1^T, \textbf s\_1^T) = (\textbf x\_i, \textbf s\_i)$

  其中：$i$ 為 $1$ 到 $T$ 的整數

- $\phi\_V$（randomly retrieve $K$ consecutive frames）：$\phi\_V(\textbf w\_1^{T - 1}, \textbf x\_1^T, \textbf s\_1^T) = (\textbf w\_{i - K}^{i - 2}, \textbf x\_{i - K}^{i - 1}, \textbf s\_{i - K}^{i - 1})$
...
The flow loss $\mathcal L\_W$ 擁有兩項，第一項是 ground truth 和估計流量之間點對點誤差，第二項則是將前一幀扭曲到下一幀時的 warping loss：
